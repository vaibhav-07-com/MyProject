  ...
  <div class="container">
    ...
    %>
      <tr>
        ...
        <td>
		  <% 
		    String status = v.getStatus(); // assuming your appointment model has getStatus()
		    if ("Pending".equalsIgnoreCase(status)) { 
		  %>
		      <!-- Approve Button -->
		      <a href="member?action=approve&vid=<%=v.getVid() %>&member_id=<%= m.getMemberId() %>" class="btn btn-success btn-sm">
		         Approve
		      </a>
		
		      <!-- Reject Button -->
		      <a href="member?action=reject&vid=<%=v.getVid() %>%member_id=<%= m.getMemberId() %>" class="btn btn-danger btn-sm ms-2">
		         Reject
		      </a>
